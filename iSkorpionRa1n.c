...
extern uint8_t payloads_iskorpion_s8000_bin[], payloads_iskorpion_s8001_bin[], payloads_iskorpion_s8003_bin[], payloads_iskorpion_t7000_bin[], payloads_iskorpion_t7001_bin[], payloads_iskorpion_t8010_bin[],  payloads_iskorpion_t8011_bin[], payloads_iskorpion_t8015_bin[];
extern unsigned payloads_iskorpion_s8000_bin_len, payloads_iskorpion_s8001_bin_len, payloads_iskorpion_s8003_bin_len, payloads_iskorpion_t7000_bin_len, payloads_iskorpion_t7001_bin_len, payloads_iskorpion_t8010_bin_len, payloads_iskorpion_t8011_bin_len, payloads_iskorpion_t8015_bin_len;
...
char xargs_cmd[0x270] = "xargs ", palerain_flags_cmd[0x30] = "shetouane";
extern char** environ;
...
#include <payloads/iskorpion_s8000.bin.h>
#include <payloads/iskorpion_s8001.bin.h>
#include <payloads/iskorpion_s8003.bin.h>
#include <payloads/iskorpion_t7000.bin.h>
#include <payloads/iskorpion_t7001.bin.h>
#include <payloads/iskorpion_t8010.bin.h>
#include <payloads/iskorpion_t8011.bin.h>
#include <payloads/iskorpion_t8015.bin.h>
...
static const char *pwnd_str = " PWND:checkra1n";
...
int pongo_cmd(usb_handle_t, char*);
static bool
send_usb_control_request(const usb_handle_t *handle, uint8_t bm_request_type, uint8_t b_request, uint16_t w_value, uint16_t w_index, void *p_data, size_t w_len, transfer_ret_t *transfer_ret) {
...
checkm8_stage_patch(const usb_handle_t *handle) {
	...
	switch (cpid) {
		case 0x8000:
			...
			data = calloc(1, payloads_iskorpion_s8000_bin_len);
			data_sz = 0;
			memcpy(data, payloads_iskorpion_s8000_bin, payloads_iskorpion_s8000_bin_len);
			data_sz += payloads_iskorpion_s8000_bin_len;
			break;
		case 0x8001:
			LOG_DEBUG("setting up stage 2 for s8001");
			data = calloc(1, payloads_iskorpion_s8001_bin_len);
			data_sz = 0;
			memcpy(data, payloads_iskorpion_s8001_bin, payloads_iskorpion_s8001_bin_len);
			data_sz += payloads_iskorpion_s8001_bin_len;
			break;
		case 0x8003:
			LOG_DEBUG("setting up stage 2 for s8003");
			data = calloc(1, payloads_iskorpion_s8003_bin_len);
			data_sz = 0;
			memcpy(data, payloads_iskorpion_s8003_bin, payloads_iskorpion_s8003_bin_len);
			data_sz += payloads_iskorpion_s8003_bin_len;
			break;
		case 0x7000:
			LOG_DEBUG("setting up stage 2 for t7000");
			data = calloc(1, payloads_iskorpion_t7000_bin_len);
			data_sz = 0;
			memcpy(data, payloads_iskorpion_t7000_bin, payloads_iskorpion_t7000_bin_len);
			data_sz += payloads_iskorpion_t7000_bin_len;
			break;
		case 0x7001:
			LOG_DEBUG("setting up stage 2 for t7001");
			data = calloc(1, payloads_iskorpion_t7001_bin_len);
			data_sz = 0;
			memcpy(data, payloads_iskorpion_t7001_bin, payloads_iskorpion_t7001_bin_len);
			data_sz += payloads_iskorpion_t7001_bin_len;
			break;
		case 0x8010:
			LOG_DEBUG("setting up stage 2 for t8010");
			data = calloc(1, payloads_iskorpion_t8010_bin_len);
			data_sz = 0;
			memcpy(data, payloads_iskorpion_t8010_bin, payloads_iskorpion_t8010_bin_len);
			data_sz += payloads_iskorpion_t8010_bin_len;
			break;
		case 0x8011:
			LOG_DEBUG("setting up stage 2 for t8011");
			data = calloc(1, payloads_iskorpion_t8011_bin_len);
			data_sz = 0;
			memcpy(data, payloads_iskorpion_t8011_bin, payloads_iskorpion_t8011_bin_len);
			data_sz += payloads_iskorpion_t8011_bin_len;
			break;
		case 0x8015:
			LOG_DEBUG("setting up stage 2 for t8015");
			data = calloc(1, payloads_iskorpion_t8015_bin_len);
			data_sz = 0;
			memcpy(data, payloads_iskorpion_t8015_bin, payloads_iskorpion_t8015_bin_len);
			data_sz += payloads_iskorpion_t8015_bin_len;
			break;
		default:
...
int pongo_cmd(usb_device_handle_t handle, char *command)
{
	uint32_t outpos = 0;
	uint32_t outlen = 0;
...
fetch_output:
	while (in_progress) {
		...
		if (ret == USB_RET_SUCCESS)
		{
			...
			if (ret == USB_RET_SUCCESS)
			{
				printf(stdout_buf + outpos, outlen);
				outpos += outlen;
				if (outpos > 0x1000)
				{
...
static void pongo_go(usb_handle_t *handle) {
	init_usb_handle(handle, APPLE_VID, PONGO_MODE_PID);
...
int main(int argc, char *argv[]) {
	printf("-=-=- iSkorpion Jailbreak -=-=-\r\n");
	printf("-=-=- www.iSkorpion.com -=-=-\r\n");
	printf("-=-=- Telegram: @Shetouane -=-=-\r\n");
	printf("\r\n");
	int ret = 0;
	...
	LOG_INFO("Setting up Pongo Device (1 Minute)");
	//sleep_ms(10000);
    pongo_go(&handle);
	LOG_INFO("Jailbreak Successful!");
normal_exit:
